...
open class KeyboardTrackingView: UIView {
	...
	/// Assign the instance of scroll view or Collection view to this variable
    /// It will move the content along with keyboard so that controls will remain visible
    ...
    @objc private func keyboardWillHide(_ notification: Notification) {
        ...
        // guard heightConstraint.constant != 0 else { return }  // RC : removed to update scrollView when keyboard is hidden
    ...
    private func animateKeyboardChange(change: Change, height: CGFloat, userInfo: [AnyHashable: Any]) {
        ...
        if let sv = scrollView {
            if change == Change.show && height > 0 {
                oldContentOffset = sv.contentOffset.y
                sv.contentOffset = CGPoint(x: sv.contentOffset.x, y: sv.contentOffset.y + height)
                
                if sv.contentOffset.y - oldContentOffset == height {
                    keyboardHeight = height
                    offsetY = sv.contentOffset.y - height
                }
            }
            
            if change == Change.hide {
                if sv.contentOffset.y <= keyboardHeight {
                    sv.contentOffset = CGPoint(x: sv.contentOffset.x, y: 0)
                }else {
                    if sv.contentSize.height - offsetY > sv.bounds.height {
                        sv.contentOffset = CGPoint(x: sv.contentOffset.x, y: sv.contentOffset.y - keyboardHeight)
                    }
                }
                keyboardHeight = 0
                offsetY = 0
